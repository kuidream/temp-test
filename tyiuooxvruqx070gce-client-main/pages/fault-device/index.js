...
Page({
  ...
  fetchDevices(storeId) {
    getDeviceList({ storeId, status: 1 })
      .then((res) => {
        const respone = JSON.parse(res)
        const list = Array.isArray(respone.data) ? respone.data : []
        const devices = list.map((item) => {
          const isFault = Number(item.deviceStatus) === 1
          return {
            id: item.deviceId,
            model: item.productType,
            name: item.deviceName,
            faultTime: item.deviceFaultTime,
            faultType: item.deviceFaultDesc,
            statusLabel: isFault ? 'device_off' : 'device_on',
            statusClass: isFault ? 'online' : 'line',
      ...
      .catch((err) => {
        console.log('fetch fault devices error:', err)
      })
  },
  toggleDetail(e) {
